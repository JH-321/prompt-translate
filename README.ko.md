...
- **한글 줄에서 Enter는 번역이 끝날 때까지 잡혀 있습니다** — 기본
  `claude -p` haiku 백엔드 기준 보통 5~9초(대부분 CLI 부팅 시간이지 모델이
  느린 게 아님). 그동안 입력창 끝에 ` …번역중` 이 표시되고, 끝나면 영어로
  바뀌며 제출됩니다. 기다리기 싫으면 Ctrl-C — 번역만 취소되고 입력줄은
  유지됩니다. 영어 줄과 `/` `!` `#` 줄은 즉시 제출됩니다. 대기가 거슬리면
  CLI 부팅이 없는 OpenRouter 백엔드(`KOEN_BACKEND=openrouter`)가 눈에 띄게
  빠릅니다.
