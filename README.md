...
동작 규칙:
- **응답은 한국어로 옵니다.** claude에는 시스템 프롬프트 1회 주입
  (`--append-system-prompt`), codex에는 번역된 턴마다 짧은 지시를 덧붙여서,
  설명 문장만 한국어로 받고 코드·경로·명령·에러 메시지는 원문 그대로
  유지됩니다. 영어 응답을 원하면 `KOEN_REPLY=en`.
...
응답을 한국어로 받으면 응답 프로즈의 출력 토큰은 영어보다 다소 늘지만,
에이전트 세션 토큰의 대부분(툴 호출, 코드, 파일 내용, 컨텍스트 재사용)은
여전히 영어/코드로 유지되므로 전체 절약 구조는 그대로입니다.
